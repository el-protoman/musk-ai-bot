...
mindsdb_pw = os.getenv('MINDSDB_PW')
...
def musk_bot():
    try:
        ...
        # Run the query on the mindsdb model
        with engine.connect() as eng:
            query = eng.execute(f"SELECT response from mindsdb.dev_chat WHERE author_username = 'junior developer' AND text= '{message}';")
            results = []
            for row in query:
                row_dict = dict(row)
